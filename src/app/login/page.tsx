...
import { Compass, Loader2, Eye, EyeOff, LogIn, ArrowRight, Mail, Lock, Zap, Sparkles, Target, TrendingUp } from "lucide-react";
    ...
    <div className="min-h-screen relative overflow-hidden bg-[#0a0a12] flex items-center justify-center p-4">
      ...
      <div className="absolute inset-0" aria-hidden>
        ...
        <motion.div animate={{ y: [0, -14, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
          className="hidden lg:block absolute top-[18%] left-[10%] px-4 py-2 rounded-xl glass border border-white/10 text-xs text-slate-300">
          <Zap className="w-3.5 h-3.5 text-amber-400 inline mr-1.5" />97% path match
        </motion.div>
        <motion.div animate={{ y: [0, 12, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut", delay: 1 }}
          className="hidden lg:block absolute bottom-[22%] right-[8%] px-4 py-2 rounded-xl glass border border-white/10 text-xs text-slate-300">
          <Sparkles className="w-3.5 h-3.5 text-emerald-400 inline mr-1.5" />Cover letter ready
        </motion.div>
      </div>
      ...
      <motion.div
        ...
        style={{ background: "linear-gradient(160deg, rgba(99,102,241,0.5), rgba(168,85,247,0.25) 40%, rgba(255,255,255,0.08) 60%, rgba(99,102,241,0.4))" }}
      >
            ...
            <div className="flex items-center justify-center mb-6">
              <div className="relative">
                <motion.div
                  ...
                  style={{ background: "conic-gradient(from 0deg, rgba(99,102,241,0.7), rgba(168,85,247,0.1), rgba(16,185,129,0.5), rgba(99,102,241,0.7))" }}
                />
                <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center shadow-lg shadow-indigo-500/30">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                <button type="submit" disabled={loading || !form.email || !form.password}
                  className="group w-full py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-400 hover:to-purple-400 text-sm font-semibold transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/25 text-white">
